...
function workerToRow(worker) {
  return {
    ...
    service_details: Array.isArray(worker.serviceDetails)
      ? worker.serviceDetails.map((d) => ({
          service: d.service || "",
          hourlyRate: parseFloat(d.hourlyRate) || null,
          tutoringByGrade: d.tutoringByGrade || [],
          tutoringSubjects: d.tutoringSubjects || [],
          tutoringGrades: d.tutoringGrades || [],
          childcareCertificateName: d.childcareCertificateName || "",
        }))
      : [],
...
function legacyWorkerToRow(worker) {
  return {
    id: worker.id,
    active: worker.active !== false,
    city: worker.city || "",
...
async function insertWorker(worker) {
  ...
  let row;
  try {
    [row] = await supabaseRequest("workers", {
      method: "POST",
      prefer: "return=representation",
      body: workerToRow(worker),
    });
  } catch (error) {
    console.warn("Supabase workers insert failed with current schema, retrying legacy row shape.", error.message);
    [row] = await supabaseRequest("workers", {
      method: "POST",
      prefer: "return=representation",
      body: legacyWorkerToRow(worker),
    });
  }
  return rowToData(row);
}
...
async function updateWorker(worker) {
  ...
  try {
    ...
    console.warn("Supabase workers update failed with current schema, retrying legacy row shape.", error.message);
    [row] = await supabaseRequest("workers", {
      method: "PATCH",
      query: `id=eq.${encodeURIComponent(worker.id)}`,
      prefer: "return=representation",
...
async function handleApi(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/api/bookings") {
    ...
    await sendBookingRequestEmail(saved);
  ...
  if (req.method === "POST" && url.pathname === "/api/workers") {
    const worker = await insertWorker(normalizeWorker(await readBody(req)));
    await sendWorkerRegistrationEmail(worker);
